...
const SubscriberCounter: React.FC<{ count?: number }> = ({ count }) => {
    if (!count || count <= 0) {
        return null;
    }
...
const PricingSection: React.FC<PricingSectionProps> = ({ onPayClick }) => {
    ...
    return (
        <div className={`grid grid-cols-1 ${packages.length > 1 ? 'md:grid-cols-3' : ''} gap-8 items-start`}>
            {isDataLoading ? (
                <p className="text-slate-500 col-span-full text-center">Planlar yükleniyor...</p>
            ) : (
                packages.map((pkg) => (
                    <div key={pkg.name} className={`bg-white p-8 rounded-2xl shadow-md border-2 transition-transform transform ${pkg.isPopular ? 'border-primary-500 scale-105' : 'border-transparent'}`}>
                        ...
                        <SubscriberCounter count={pkg.subscribers} />
                        ...
                        >
                           {onPayClick 
                                ? 'Planı Seç'
                                : paymentLoading === pkg.name ? 'İşleniyor...' : success === pkg.name ? 'Ödeme Başarılı!' : `Stripe ile Öde`
                           }
                        </button>
                    </div>
                ))
